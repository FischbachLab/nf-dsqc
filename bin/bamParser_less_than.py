...
def usage():
    ...
    """,
        epilog="""Examples:
    python bamParser.py -bam sample.sortedByCoord.bam -id 98 -aln_len 100 -out filtered.bam
    or
    python bamParser.py -bam sample.sortedByCoord.bam -id 98 -aln_len 100 -reads readnames.txt -out filtered.bam
    or
    python bamParser.py -bam sample.sortedByCoord.bam -id 98 -aln_len 100 -reads readnames.txt -out filtered.tsv -out_fmt tsv
    """,
    )
    # Required
    p.add_argument(
    ...
    p.add_argument(
        ...
        action="store",
        type=float,
        help="minimum percent nucleotide identity (inclusive).",
        default=100,
    )
    p.add_argument(
        "-aln_len",
        ...
        action="store",
        type=float,
        help="minimum percent read alignment length (inclusive).",
        default=100,
    )
    p.add_argument(
        "-reads",
    ...
    p.add_argument(
        ...
        dest="plot",
        action="store_true",
        help="plot percent id and alignment length histograms",
        default=False,
    )
...
def acceptable_alignment(aln_pid, aln_len, min_pid, min_paln):
    ...
    # not perfect
    # return (aln_pid != min_pid ) or ( aln_len != min_paln)
...
def filter_bam(
    ...
    for aln in bam_in.fetch(until_eof=True):
        ...
        if not aln.has_tag("NM"):
            continue
        if aln.infer_read_length() is None:
            continue
...
if __name__ == "__main__":
    ...
    (aln_remaining, total_aln) = filter_bam(
        bamfile_name,
        min_id,
        min_aln_len,
        output_file_name,
    ...
    print(
        f"Retained alignments: {aln_remaining}/{total_aln}. [~{round(aln_remaining*100/total_aln, 3)}%]"
    )
    sys.exit(0)
